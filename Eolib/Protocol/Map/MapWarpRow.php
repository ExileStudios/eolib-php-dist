...
use Eolib\Protocol\Map\MapWarpRowTile;
...
class MapWarpRow
{
    ...
    /** @return MapWarpRowTile[] */
    public function getTiles(): array
    {
        return $this->tiles;
    }

    /** @param MapWarpRowTile[] $tiles */
     ...
     * @param MapWarpRow $data The data to serialize.
     */
    public static function serialize(EoWriter $writer, MapWarpRow $data): void {
        if ($data->getY() == null)
        {
        ...
        {
            MapWarpRowTile::serialize($writer, $data->getTiles()[$i]);
     ...
     * @return MapWarpRow The deserialized data.
     */
    public static function deserialize(EoReader $reader): MapWarpRow
    {
        $data = new MapWarpRow();
        ...
        try {
            ...
            for ($i = 0; $i < $data->getTilesCount(); $i++)
            {
                $data->tiles[] = MapWarpRowTile::deserialize($reader);
            }
    ...
    public function __toString(): string {
        return "MapWarpRow(byteSize=$this->byteSize, y=".var_export($this->y, true).", tiles=".var_export($this->tiles, true).")";
    }

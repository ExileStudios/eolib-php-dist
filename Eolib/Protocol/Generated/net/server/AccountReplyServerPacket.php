...
use Eolib\Protocol\Generated\Net\server\AccountReply;
...
class AccountReplyServerPacket
{
    ...
    public static function serialize(EoWriter $writer, AccountReplyServerPacket $data): void {
        ...
        if ($data->replyCode === 0)
        {
            ...
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be null for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
        }
        elseif ($data->replyCode === AccountReply::EXISTS)
        {
            ...
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataExists for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
        ...
        elseif ($data->replyCode === AccountReply::NOTAPPROVED)
        {
            ...
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataNotApproved for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
        ...
        elseif ($data->replyCode === AccountReply::CREATED)
        {
            ...
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataCreated for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
            ReplyCodeDataCreated::serialize($writer, $data->replyCodeData);
        }
        ...
        elseif ($data->replyCode === AccountReply::CHANGEFAILED)
        {
            ...
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataChangeFailed for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
            ReplyCodeDataChangeFailed::serialize($writer, $data->replyCodeData);
        }
        elseif ($data->replyCode === AccountReply::CHANGED)
        {
            if (!($data->replyCodeData instanceof ReplyCodeDataChanged))
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataChanged for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
            ReplyCodeDataChanged::serialize($writer, $data->replyCodeData);
        }
        elseif ($data->replyCode === AccountReply::REQUESTDENIED)
        {
            if (!($data->replyCodeData instanceof ReplyCodeDataRequestDenied))
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataRequestDenied for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
            ReplyCodeDataRequestDenied::serialize($writer, $data->replyCodeData);
        }
        ...
        else
        {
            ...
            {
                throw new \Eolib\Protocol\SerializationError("Expected replyCodeData to be of type ReplyCodeDataDefault for replyCode " . AccountReply($data->replyCode)->name . ".");
            }
    ...
    public static function deserialize(EoReader $reader): AccountReplyServerPacket
    {
        ...
        try {
            ...
            $data->replyCode = AccountReply($reader->getShort());
            if ($data->replyCode === 0)
            {
            ...
            elseif ($data->replyCode === AccountReply::CREATED)
            {
                $data->replyCodeData = ReplyCodeDataCreated::deserialize($reader);
            }
            elseif ($data->replyCode === 4)
            {
                $data->replyCodeData = null;
            ...
            elseif ($data->replyCode === AccountReply::CHANGEFAILED)
            {
                $data->replyCodeData = ReplyCodeDataChangeFailed::deserialize($reader);
            }
            elseif ($data->replyCode === AccountReply::CHANGED)
            {
                $data->replyCodeData = ReplyCodeDataChanged::deserialize($reader);
            }
            elseif ($data->replyCode === AccountReply::REQUESTDENIED)
            {
                $data->replyCodeData = ReplyCodeDataRequestDenied::deserialize($reader);
            }
            elseif ($data->replyCode === 8)
            {
                $data->replyCodeData = null;
    ...
    public function __toString(): string {
        return "AccountReplyServerPacket(byteSize=' . $this->byteSize . '', replyCode=' . $this->replyCode . '', replyCodeData=' . $this->replyCodeData . ')";
    }
...
class ReplyCodeDataDefault implements ReplyCodeData
{
    ...
    public static function serialize(EoWriter $writer, ReplyCodeDataDefault $data): void {
        if ($data->sequenceStart === null)
        {
            throw new SerializationError('sequenceStart must be provided.');
        }
        $writer->addChar($data->sequenceStart);
    ...
    public static function deserialize(EoReader $reader): ReplyCodeDataDefault
    {
        ...
        try {
            $reader_start_position = $reader->getPosition();
            $data->sequenceStart = $reader->getChar();
            $reader->getString();
     ...
     */
    public function __toString(): string {
        return "ReplyCodeDataDefault(byteSize=' . $this->byteSize . '', sequenceStart=' . $this->sequenceStart . ')";
    }

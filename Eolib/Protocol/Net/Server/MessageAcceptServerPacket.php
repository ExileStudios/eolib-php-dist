...
class MessageAcceptServerPacket
{
    ...
    public static function serialize(EoWriter $writer, MessageAcceptServerPacket $data): void {
        ...
        if (count($data->messages) != 4)
        {
            throw new SerializationError("Expected length of messages to be exactly 4, got " . count($data->messages) . ".");
        }
        for ($i = 0; $i < 4; $i++)
        {
            if ($i > 0)
    ...
    public function __toString(): string {
        return "MessageAcceptServerPacket(byteSize=$this->byteSize, messages=".var_export($this->messages, true).")";
    }
